      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="grid grid-cols-1 divide-y divide-border">
          ...
          <InfoRow icon={<User className="h-4 w-4 text-muted-foreground"/>} label="Owner">
            <Link href={`/customers/${license.discordId}`} className="flex items-center gap-2 hover:underline" onClick={(e) => e.stopPropagation()}>
           ...
           <InfoRow icon={<CalendarPlus className="h-4 w-4 text-muted-foreground"/>} label="Created">
            <span>{format(new Date(license.createdAt), "PPP")}</span>
          </InfoRow>
          <InfoRow icon={<Calendar className="h-4 w-4 text-muted-foreground"/>} label="Expires">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-2 rounded-md border p-4">
                <h4 className="flex items-center gap-2 text-sm font-semibold"><Globe className="h-4 w-4" /> Allowed IPs ({license.allowedIps.length}/{license.maxIps === -1 ? '∞' : license.maxIps})</h4>
