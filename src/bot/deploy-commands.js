...
function deploy() {
    ...
    if (!config.enabled) {
        console.log("[INFO] Bot is disabled, skipping command deployment.");
        return;
    }
    ...
    const enabledCommands = commandKeys.filter(key => config.commands[key] === true);
    ...
    for (const file of commandFiles) {
        try {
            ...
            if (command.data && command.data.name) {
                const commandNameCamelCase = command.data.name.replace(/-([a-z])/g, g => g[1].toUpperCase());
                if (enabledCommands.includes(commandNameCamelCase)) {
